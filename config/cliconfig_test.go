...
var (
	...
	notificationFilePath       = wdTestPath + "appconfig.change-test_" + randomString() + ".cfg"
	noChangeFilePath           = wdTestPath + "appconfig.no-notify_" + randomString() + ".cfg"
	removeFilePath             = wdTestPath + "appconfig.remove_" + randomString() + ".cfg"
	hashTestPath               = wdTestPath + "appconfig.hash-err_" + randomString() + ".cfg"
...
func TestCLIConfigPathChangeNotification(t *testing.T) {
	...
	t.Run("HashErrors", func(t *testing.T) {
		...
		time.Sleep(1 * time.Millisecond)
		getFileHash = errHashFn
		writeToFile(hashTestPath, notificationInitialContent)
		time.Sleep(1 * time.Millisecond)
		assert.Contains(t, buf.String(), expectedErr.Error())

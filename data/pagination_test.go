...
func TestGetCursor(t *testing.T) {
	paginateable := BasePaginateable{}
	paginateable.CreatedAt = time.Now()
	paginateable.UpdatedAt = time.Now()
	paginateable.ID = xid.New()
	cursor, err := paginateable.GetCursor()
	assert.Nil(t, err)
	assert.NotNil(t, cursor)
}

func TestDefaultQuickFix(t *testing.T) {
	paginateable := BasePaginateable{}
	assert.Equal(t, true, paginateable.QuickFix())
	paginateable = BasePaginateable{}
	paginateable.CreatedAt = time.Now()
	assert.Equal(t, true, paginateable.QuickFix())
	paginateable = BasePaginateable{}
	...
	paginateable.UpdatedAt = time.Now()
	paginateable.ID = xid.New()
	assert.Equal(t, false, paginateable.QuickFix())
}
